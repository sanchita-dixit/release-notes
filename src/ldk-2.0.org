...
cloud. The AWS cloud is designed in such a way that the virtual
machines hosting the labs are not accessible to the outside
world. AWS already imposes several security features in each 
individual VM created, on top of which another layer of 
security in the form of firewalls and reverse 
proxy is built such that the traffic is 
directed through a single machine called =router=.
